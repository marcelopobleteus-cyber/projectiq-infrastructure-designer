...
interface FieldBottomTabsProps {
  projectId?: string
  onToggleDrawer: () => void
  isDrawerOpen: boolean
}

export default function FieldBottomTabs({
  projectId,
  onToggleDrawer,
  isDrawerOpen,
}: FieldBottomTabsProps) {
  ...
  const tabs = [
    {
      id: 'overview',
      label: 'Overview',
      href: `/field/projects/${defaultProjectId}/overview`,
      active: pathname.includes('/overview') || pathname === '/field/projects',
      icon: (
    ...
    },
    {
      id: 'cameras',
      label: 'Cameras',
      href: `/field/projects/${defaultProjectId}/cameras`,
    ...
    },
    {
      id: 'tasks',
      label: 'Tasks',
      href: `/field/projects/${defaultProjectId}/tasks`,
    ...
    },
    {
      ...
      label: 'Time Card',
      href: `/field/time`,
      active: pathname.startsWith('/field/time'),
      icon: (
  ...
  return (
    <nav className="h-16 bg-[var(--surface-1)] border-t border-[var(--border)] fixed bottom-0 left-0 right-0 z-40 flex items-center justify-around px-2 shrink-0 font-sans shadow-xl">
      ...
      >
        ...
        <span className="text-[10px] tracking-tight font-medium leading-none">Menu</span>
      </button>
    </nav>
  )
}
