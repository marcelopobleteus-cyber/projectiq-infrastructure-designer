      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-4 scrollbar-thin select-none">
        {categories.map((cat, idx) => (
          <div key={idx} className="space-y-1">
            <span className="text-[9px] font-bold text-slate-500 uppercase tracking-widest block px-3 mb-1.5">
              {cat.label}
            </span>
            <div className="space-y-0.5">
              {cat.items.map((sec) => (
                <Link
                  key={sec.id}
                  href={sec.href}
                  className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-[11px] transition-all border ${
                    sec.active
                ...
                >
                  <span className={sec.active ? 'text-indigo-400' : 'text-slate-500'}>
                    {sec.icon}
                  </span>
                  {sec.label}
                </Link>
