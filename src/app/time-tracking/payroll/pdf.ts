...
import { buildStatementPdf, buildPayrollReportPdf } from '@/lib/payStatementPdf'
...
/** Reporte de todos los empleados del periodo, en un solo PDF. */
export async function generatePayrollReportPdf(
  ...
  const membership = memberRows?.[0]
  if (!membership) return { error: 'No organization for the current user.' }
  // El reporte global lleva tarifas y costos de todo el equipo.
  if (membership.role !== 'owner' && membership.role !== 'admin') {
    return { error: 'Payroll reports are limited to owners and admins.' }
  }

  const orgId = membership.organization_id

  const [{ data: org }, summary] = await Promise.all([
    supabase.from('organizations').select('name, address').eq('id', orgId).single(),
    supabase.rpc('payroll_summary', { p_organization_id: orgId, p_from: from, p_to: to }),
  ])

  const rows = summary.data ?? []
  if (rows.length === 0) return { error: 'No hours in this period.' }

  const bytes = await buildPayrollReportPdf({
    organizationName: org?.name ?? 'Company',
    organizationAddress: org?.address ?? null,
    from,
    to,
  ...
  return {
    base64: Buffer.from(bytes).toString('base64'),
    fileName: `payroll_report_${from}_${to}.pdf`,
  }
}
